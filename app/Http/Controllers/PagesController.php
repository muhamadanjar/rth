...
use App\User;
use App\GeoTag;
use App\TRTHPKOTBOGOR;
use App\RTHCIFOR;
use App\RTHKEBUNRAYA;
...
class PagesController extends Controller {
	...
	public function formCaptureGet($objectid){
		return view('page.form.takephoto',compact('objectid'));
	}
	...
	public function editGeotagFoto(Request $request) {
		...
		$foto = $request->foto;
		$pathFilename = 'images/rth/'.$foto;
		if($request->ajax()){
            $GeoTag = TRTHPKOTBOGOR::where('objectid' , '=', $objectid)->first();
            $GeoTag->image_link = $pathFilename;
	...
	public function editGeotagFotoCifor(Request $request) {
		...
		if($request->ajax()){
            $GeoTag = RTHCIFOR::where('objectid' , '=', $objectid)->first();
            $GeoTag->image_link = $pathFilename;
			//$GeoTag->personil = \Auth::user()->name;
			//$GeoTag->tanggal = Carbon::now();
			$GeoTag->save();
	...
	public function editGeotagFotoKebunRaya(Request $request) {
		...
		if($request->ajax()){
            $GeoTag = RTHKEBUNRAYA::where('objectid' , '=', $objectid)->first();
            $GeoTag->image_link = $pathFilename;
			//$GeoTag->personil = \Auth::user()->name;
			//$GeoTag->tanggal = Carbon::now();
			$GeoTag->save();
